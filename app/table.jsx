  ...
  return (
    <div className="panel">
      <div className="panel-head">
        <div className="panel-title">Comparison of {pillar.layout === 'grouped' ? 'sub-pillar indicators' : 'indicators'} · <span style={{ color: countryColor, fontWeight: 600 }}>{country}</span></div>
    ...
    <div className={'defpanel-overlay' + (open ? ' open' : '')} onClick={onClose}>
      <div className="defpanel" onClick={e => e.stopPropagation()}>
        {row && <>
          ...
          {row.hideScores ? (
            <div className="defpanel-section">
              <div className="defpanel-note">ℹ {row.breakdownNote || 'This indicator has multiple breakdown categories. Select a specific category in the table to explore values.'}</div>
            </div>
          ) : (
            <div className="defpanel-scores">
              ...
              <div className="dps"><div className="dps-k">YoY change</div><div className="dps-v" style={{ color: (delta == null || Math.abs(delta) < 0.001) ? 'var(--color-neutral)' : delta > 0 ? 'var(--color-up)' : 'var(--color-down)' }}>
                {delta == null ? '—' : Math.abs(delta) < 0.001 ? '— 0' : (delta > 0 ? '▲ ' + fmtScore(delta) : '▼ -' + fmtScore(Math.abs(delta)))}
              </div></div>
            </div>
          )}

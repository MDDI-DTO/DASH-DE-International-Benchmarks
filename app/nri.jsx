...
function NRIHeaderCard({ domain, country, year, overall, prev, series, pillarColor, accent, eyebrow }) {
  ...
  const def = stripPublishedBy(defOf('NRI', 'Network Readiness Index', 'Overall', '') || (window.PILLAR_CONTEXT && window.PILLAR_CONTEXT['nri']) || '');
  ...
  return (
    <div className="panel header-card imd-header">
      ...
      <div className="hc-left">
        ...
        {def && <div className="hc-def">{def} <ExtLinkIcon url="https://networkreadinessindex.org/" color={ac} /></div>}
        <div className="hc-source"><span className="src-link" style={{ color: ac }}>Portulans Institute</span> · Annual</div>
      </div>
...
function NRIView({ domain, years, country, year, onPickCountry, onYear }) {
  ...
  // Country change must not clear the drill-down (third-tier) selection — clicking
  // a country bar in the drill chart should only re-highlight that country. Only a
  // pillar switch or year change (new dataset) clears the sub-indicator selection.
  React.useEffect(() => { setThird(null); setSubPillarFilter('all'); }, [pillar, year]);
      ...
      <div className="panel-row grow">
        <div className="panel">
          <div className="panel-head panel-head-row">
            <div className="ph-titles">
              {drill && (
                <button type="button" className="indicator-back-btn" style={{ color: pc }} onClick={() => setThird(null)}>← Back to sub-pillar overview</button>
              )}
              <div className="panel-title">{drill
                ? <><span className="ind-chip">{third.label}</span> · {profAll ? 'All' : 'Singapore'} Country Rank Comparison</>
                : <>Comparison of sub-pillars across countries · <span style={{ color: pc, fontWeight: 600 }}>{country}</span></>}</div>
              <div className="panel-sub">Click on a country to explore its sub-indicators</div>
            </div>
            <div className="chart-head-ctrls">
          ...
          {drill
            ? <div className={'panel-body' + (profAll ? ' body-scroll' : '')}>
                <CountryBarChart rows={drillRows} selectedCountry={country} onPick={onPickCountry} fixedRows={profAll} />
              </div>
        ...
        <div className="panel">
          <div className="panel-head">
            <div className="panel-title">Comparison of sub-indicators across countries · <span style={{ color: pc, fontWeight: 600 }}>{country}</span></div>
            <div className="panel-sub">Click on a third-tier indicator to explore its country comparison</div>
          </div>

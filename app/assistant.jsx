...
/* G4 — the live backend (Supabase Edge Function) is not wired up yet. While false,
   the launcher shows a "coming soon" clock badge and the panel shows a friendly
   placeholder instead of the chat UI. Flip to true once the backend is ready. */
...
function DASHDEAssistant() {
  ...
  React.useEffect(() => {
    try {
      // OV10 — while the assistant isn't live, greet with the WIP bubble on load, then
      // fade it after a few seconds (it still reappears on hover).
      if (!ASSISTANT_LIVE) {
        setHint(true);
  ...
  return (
    <>
      {/* hint / work-in-progress bubble — hover (desktop) or first-load then fade (OV10) */}
      {!open && (hint || hover) && (
        <div className="assist-hint">
          {!ASSISTANT_LIVE
            ? <div className="assist-hint-txt">🚧 Still learning the data… check back soon! 🤓</div>
            : <>
      ...
      {!open && (
        <button className="assist-launcher" onClick={openPanel} aria-label="Open DASH-DE Assistant"
          ...
          {ASSISTANT_LIVE && dot && <span className="assist-dot" />}
        </button>
      )}
        ...
        <div className="assist-panel" role="dialog" aria-label="DASH-DE Assistant">
          <div className="assist-head">
            <div className="assist-head-titles">
              ...
              <div className="assist-head-sub">{ASSISTANT_LIVE ? 'Ask about any ranking, pillar, or trend' : 'Coming soon'}</div>
            </div>
            <button className="assist-close" onClick={() => setOpen(false)} aria-label="Close">✕</button>
          </div>

          {!ASSISTANT_LIVE ? (
            <div className="assist-body assist-soon-body">
              <div className="assist-msg assistant">
                <span className="assist-av">✦</span>
                <div className="assist-bubble">
                  <p className="assist-line">Hey there! 👋</p>
                  <p className="assist-line">I’m still getting my data briefing ready — I’ll be at your service soon! In the meantime, the dashboard has everything you need. 📊</p>
                  <p className="assist-line">Check back soon!</p>
                </div>
              </div>
            </div>
          ) : (
          ...
          <div className="assist-disclaimer">Answers are generated from live Supabase data — verify against source publications. This conversation is session-only and is not stored once you close the panel.</div>
          </>
          )}
        </div>
      )}

...
function GAIView({ domain, years, country, year, onPickCountry, onYear }) {
  ...
  // R11 Change 1 — GAI tab always has one pillar + one sub-pillar active (no "All"
  // aggregate state). Defaults to the first pillar's first sub-pillar on entry.
  const [dimFilter, setDimFilter] = React.useState(() => (defaultSel ? defaultSel.dim : (dims[0] || null)));  // active pillar (cards + table + dropdown source)
  const [subFilter, setSubFilter] = React.useState(() => (defaultSel ? defaultSel.sub : null));  // Sub-pillar dropdown value
  const [selected, setSelected] = React.useState(defaultSel); // rank-profile chart subject

  // reset to the default (first pillar, first sub-pillar) whenever country/year change,
  // establishing a clean synced state — mirrors the reset-on-tab-entry requirement.
  React.useEffect(() => {
    setSelected(defaultSel);
    setDimFilter(defaultSel ? defaultSel.dim : (dims[0] || null));
    setSubFilter(defaultSel ? defaultSel.sub : null);
  ...
  const subPillarOptions = React.useMemo(
    () => (dimFilter ? gaiPillarSubs(dimFilter, country, year).map(s => s.sub) : []),
    ...
    <>
      ...
      <div className="panel-row grow">
        <div className="panel gai-chart">
          <div className="panel-head panel-head-row">
            <div className="ph-titles">
              <div className="panel-title">Comparison of {dimFilter}{sel && sel.sub ? <> · <span style={{ color: GAI_ACCENT, fontWeight: 600 }}>{sel.sub}</span></> : ''} across countries · <span style={{ color: GAI_DARK, fontWeight: 700 }}>{country}</span></div>
        ...
        <div className="panel">
          <div className="panel-head">
            <div className="panel-title">Comparison of {dimFilter} sub-pillars · <span style={{ color: GAI_DARK, fontWeight: 700 }}>{country}</span></div>

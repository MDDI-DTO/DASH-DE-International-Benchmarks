...
function App({ dataSource }) {
  ...
  function switchDomain(d) {
    ...
    if (d === 'AI') { setAiLevel1('gai'); setGariPillar(null); setAiPillarId(((PILLARS['AI'] || [])[0] || {}).id); }
  }
  ...
  function navigateFromOverview(pub, pillarLabel) {
    if (pub === 'IMD WCY') {
      ...
      const valid = ['Government', 'Technology Sector', 'Data and Infrastructure'];
      setDomain('AI'); setAiLevel1('gari');
      setGariPillar(pillarLabel && valid.includes(pillarLabel) ? pillarLabel : null);
    }
  }
  function pickSub(r) {
    setSelectedSub(prev => {
    ...
    <div className={'stage' + (isScroll ? ' stage-scroll' : '') + (fluid ? ' stage-fluid' : '')}>
      ...
      {domain === 'AI' && (
        <div className="level1-tabs">
          ...
          <button className={'l1' + (aiLevel1 === 'gari' ? ' active' : '')} style={{ '--tab-accent': PUB_COLOR['Government AI Readiness Index'] }}
            onClick={() => { setAiLevel1('gari'); setGariPillar(null); }}>Government AI Readiness Index</button>
        </div>
      )}
      ...
      {domain === 'AI' && aiLevel1 === 'gari' && (
        <div className="level2-tabs" style={{ '--tab-accent': PUB_COLOR['Government AI Readiness Index'] }}>
          <button className={'l2' + (!gariPillar ? ' active' : '')} onClick={() => setGariPillar(null)}>

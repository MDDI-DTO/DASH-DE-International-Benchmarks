...
function App({ dataSource }) {
  ...
  return (
    <div className={'stage' + (isScroll ? ' stage-scroll' : '') + (fluid ? ' stage-fluid' : '')}>
      ...
      <div className={'canvas-sizer' + (isScroll ? ' sizer-clip' : '')} style={fluid ? { width: '100%', height: '100vh' } : { width: CANVAS_W * scale, height: contentH * scale }}>
      <div className={'canvas' + (isGAI ? ' canvas-nri' : '') + (isOverview ? ' canvas-overview' : '') + ((isOverallIMD || isNRIPage || isGARIPage) ? ' canvas-overall' : '') + ((isNRIPage || isGARIPage) ? ' canvas-nri2' : '') + (fluid ? ' canvas-fluid' : '')} ref={canvasRef}
        style={fluid ? undefined : { transform: `scale(${scale})`, transformOrigin: 'top left' }}>
      {/* top bar */}

...
function aiiCountries(year) {
  const set = new Set();
  for (const r of RECS) if (r.p === AII_PUB && r.y === year && r.c) set.add(r.c);
...
function aiiSubRows(pillar, country, year) {
  const map = new Map();
  for (const r of RECS) if (r.p === AII_PUB && r.i === pillar && r.c === country && r.y === year && r.s) map.set(r.s, r);
  return [...map.values()];
}
/* distinct sub-indicator names tracked for a pillar in a given year (country-
   independent — this is about coverage, not one country's values). */
function aiiPillarSubIndicators(pillar, year) {
  const set = new Set();
  for (const r of RECS) if (r.p === AII_PUB && r.i === pillar && r.y === year && r.s) set.add(r.s);
  return [...set];
}
...
function aii2025Cards(country, year) {
  ...
  return aiiPillars(year).map(p => {
    const names = aiiPillarSubIndicators(p, year);
    return {
...
/* country comparison for one pillar+sub-indicator — same unit only (never mix
   units on one chart), deduplicated per country, sorted by score descending. */
function aiiCompareCountries(pillar, sub, year) {
  const map = new Map();
  for (const r of RECS) if (r.p === AII_PUB && r.i === pillar && r.s === sub && r.y === year && r.sc != null) map.set(r.c, r);
  let rows = [...map.values()];
  ...
  for (const r of RECS) if (r.p === AII_PUB && r.i === pillar && r.s === sub && r.y === year - 1 && r.sc != null) prevMap.set(r.c, r.sc);
  return rows.map((r, i) => ({ country: r.c, score: r.sc, unit: r.u, pos: i + 1, prevScore: prevMap.has(r.c) ? prevMap.get(r.c) : null }));
}

Object.assign(window, {
  AII_PUB, AII_PILLARS, AII_ICONS, aiiYears, aiiCountries, aiiPillars,
  aiiSubRows, aiiPillarSubIndicators, aii2025Cards, aiiSubTable, aiiCompareCountries,
});

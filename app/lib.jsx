...
let DEFS = {};
let ISO  = {};
/* Supplemental country→ISO-alpha-2 flag map. The bundled seed `iso` covers the
   IMD/NRI country lists; GARI (and a few GAI) use the ISO-3166 long-form names
   (often with a trailing "(the)") and diacritics that aren't in the seed map.
   Merged over the seed map at load so flagUrl() resolves every GAI/GARI country. */
const ISO_EXTRA = {
  // --- GAI gaps ---
  'Iran': 'ir', 'Iraq': 'iq', 'Russia': 'ru', 'The Netherlands': 'nl', 'Vietnam': 'vn',
  // --- GARI long-form + (the) variants + diacritics ---
  'Afghanistan': 'af', 'Andorra': 'ad', 'Antigua and Barbuda': 'ag',
  'Bahamas': 'bs', 'Bahamas (the)': 'bs', 'Barbados': 'bb', 'Belarus': 'by', 'Belize': 'bz',
  'Bhutan': 'bt', 'Bolivia': 'bo', 'Brunei Darussalam': 'bn',
  'Central African Republic': 'cf', 'Central African Republic (the)': 'cf', 'Chad': 'td',
  'Comoros': 'km', 'Comoros (the)': 'km', 'Congo': 'cg', 'Congo (the)': 'cg', 'Cuba': 'cu',
  "C\u00f4te D'Ivoire": 'ci', "C\u00f4te d'Ivoire": 'ci',
  "Democratic People's Republic of Korea": 'kp', "Democratic People's Republic of Korea (the)": 'kp',
  'Democratic Republic of the Congo': 'cd', 'Democratic Republic of the Congo (the)': 'cd',
  'Djibouti': 'dj', 'Dominica': 'dm', 'Dominican Republic (the)': 'do',
  'Equatorial Guinea': 'gq', 'Eritrea': 'er', 'Eswatini': 'sz', 'Fiji': 'fj', 'Gabon': 'ga',
  'Gambia': 'gm', 'Gambia (Republic of The)': 'gm', 'Gambia (the)': 'gm', 'Grenada': 'gd',
  'Guinea': 'gn', 'Guinea Bissau': 'gw', 'Guinea-Bissau': 'gw', 'Guyana': 'gy', 'Haiti': 'ht',
  'Kiribati': 'ki', "Lao People's Democratic Republic (the)": 'la', 'Lebanon': 'lb',
  'Liberia': 'lr', 'Libya': 'ly', 'Liechtenstein': 'li', 'Maldives': 'mv',
  'Marshall Islands': 'mh', 'Marshall Islands (the)': 'mh', 'Micronesia (Federated States of)': 'fm',
  'Monaco': 'mc', 'Myanmar': 'mm', 'Nauru': 'nr', 'Netherlands (the)': 'nl',
  'Niger': 'ne', 'Niger (the)': 'ne', 'Palau': 'pw', 'Papua New Guinea': 'pg', 'Philippines (the)': 'ph',
  'Republic of Korea (the)': 'kr', 'Republic of Moldova (the)': 'md', 'Russian Federation (the)': 'ru',
  'Saint Kitts and Nevis': 'kn', 'Saint Lucia': 'lc', 'Saint Vincent and the Grenadines': 'vc',
  'Samoa': 'ws', 'San Marino': 'sm', 'Sao Tome and Principe': 'st', 'Seychelles': 'sc',
  'Sierra Leone': 'sl', 'Solomon Islands': 'sb', 'Somalia': 'so', 'South Sudan': 'ss',
  'State of Palestine': 'ps', 'Sudan': 'sd', 'Sudan (the)': 'sd', 'Suriname': 'sr', 'Swaziland': 'sz',
  'Syrian Arab Republic': 'sy', 'Syrian Arab Republic (the)': 'sy', 'Tajikistan': 'tj', 'Tanzania': 'tz',
  'The former Yugoslav Republic of Macedonia': 'mk', 'Timor-Leste': 'tl', 'Togo': 'tg', 'Tonga': 'to',
  'Turkmenistan': 'tm', 'Tuvalu': 'tv', 'United Arab Emirates (the)': 'ae',
  'United Kingdom of Great Britain and Northern Ireland': 'gb',
  'United Kingdom of Great Britain and Northern Ireland (the)': 'gb',
  'United States of America (the)': 'us', 'Vanuatu': 'vu',
  'Venezuela (Bolivarian Republic of)': 've', 'Venezuela, Bolivarian Republic of': 've', 'Yemen': 'ye',
};
...
const PUB_FULL = {
  ...
  'Stanford AI Index': 'Stanford Global AI Vibrancy Tool',
};
const PUB_SHORT = { 'IMD WCY': 'IMD WCY', 'IMD WDC': 'IMD WDC', 'NRI': 'NRI', 'Government AI Readiness Index': 'GARI', 'Global AI Index': 'GAI', 'Stanford AI Index': 'AII' };
const PUB_URL = {
  ...
  'Global AI Index': 'https://observer.co.uk/data/global-ai',
  'Stanford AI Index': 'https://hai.stanford.edu/ai-index',
};

// R14 Change 1 — explicit tab order for the Digital Economy nav (Overall → Technology → People → Impact → Governance); not derived from Supabase query order.
const NRI_PILLARS = ['Technology', 'People', 'Impact', 'Governance'];

const PILLARS = {
  'Digital Economy': [
    // R14 Change 1 — explicit tab order: Tech Infrastructure → Overall (IMD WDC, rendered separately in app.jsx) → Technology → Knowledge → Future Readiness.
...
async function loadData() {
  ...
  ISO = Object.assign({}, seed.iso || {}, ISO_EXTRA); // static lookup — seed map + GAI/GARI long-form names
...
/* R7 G2 — generic YoY rank attachment for country bar-chart rows. Looks up each
   country's prior-year rank for the same pub/indicator/sub/third combo via the
   existing compare index; if no matching row is found (restructured pillar,
   first year of data, GARI era boundary, etc.) prevRank stays null and the bar
   chart simply omits the indicator — this alone satisfies the "don't show YoY
   across incomparable GARI eras" rule without a separate guard. */
function attachYoY(rows, pub, indicator, year, sub, third) {
  const prev = compareTop(pub, indicator, year - 1, sub, third, null);
  const map = {};
  prev.forEach(r => { map[r.country] = r.rank; });
  return rows.map(r => ({ ...r, prevRank: map[r.country] != null ? map[r.country] : null }));
}
/* GARI's overall ranking doesn't route through the generic compare index (it scans
   RECS directly, ignoring sub_indicator), so it gets a dedicated YoY attachment that
   reuses gariOverallRanking for the prior year. Overall rank is the one value
   confirmed cross-era comparable, so this also works across the 2024→2025 boundary. */
function attachYoYGariOverall(rows, year) {
  const prev = gariOverallRanking(year - 1);
  const map = {};
  prev.forEach(r => { map[r.country] = r.rank; });
  return rows.map(r => ({ ...r, prevRank: map[r.country] != null ? map[r.country] : null }));
}
...
/* Single shared ordinal formatter — number + suffix with NO space between them
   (e.g. 1 -> "1st", 22 -> "22nd"). Every rank value rendered anywhere in the
   dashboard routes through this so no column shows a bare integer or a spaced
   suffix. Returns an em-dash for null/undefined. */
function formatOrdinal(n) {
  if (n == null) return '—';
  return `${n}${ordinal(n)}`;
}
/* Round-3 Change 1 — strip a trailing "Published [annually] by {publisher}." sentence
   from a definition paragraph (the source line already attributes the publisher). */
function stripPublishedBy(text) {
  if (!text) return text;
  return text.replace(/\s*Published(\s+annually)?\s+by\s+[^.]*\.?\s*$/i, '').trim();
}
...
function useIndicatorData(pillar, country, year) {
  // R12 Change 4 — GAI and NRI views fetch their own data synchronously from the
  // in-memory index and never read this hook's `data`; letting this hook still run
  // its 160ms loading flicker for them swapped the whole panel out for skeletons on
  // every country click, which collapsed the scroll container's height and caused
  // the page to visibly jump to the top. Skip the fetch/loading cycle entirely for
  // those two layouts so a country click never disturbs the shared skeleton branch.
  const skip = !!(pillar && (pillar.layout === 'gai' || pillar.layout === 'nri'));
  const [state, setState] = React.useState({ loading: !skip, error: null, data: skip ? {} : null });
  React.useEffect(() => {
    if (!pillar) { setState({ loading: false, error: null, data: null }); return; }
    if (skip) { setState({ loading: false, error: null, data: {} }); return; }
    let alive = true;
  ...
  }, [pillar && pillar.id, country, year, skip]);
  return state;
}
...
function nriPillarTable(P, country, year) {
  ...
  // OV5 — the bold pillar row above IS the pillar's Overall; the separate
  // "Overall – {pillar}" row duplicated it, so it is no longer emitted.
...
function gaiTableModel(country, year, dimFilter) {
  ...
  if (all) {
    ...
    const ovp = gaiOverallRow(country, py);
    rows.push({ kind: 'overall', label: 'Overall', dim: 'Overall', rank: ov ? ov.r : null, score: ov ? ov.sc : null, prevRank: ovp ? ovp.r : null, prevScore: ovp ? ovp.sc : null });
  }
  ...
  for (const dim of dims) {
    ...
    const pop = gaiPillarOverall(dim, country, py);
    rows.push({ kind: 'poverall', label: 'Overall', dim, rank: po ? po.r : null, score: po ? po.sc : null, prevRank: pop ? pop.r : null, prevScore: pop ? pop.sc : null });
    const prevSubs = {};
    for (const s of gaiPillarSubs(dim, country, py)) prevSubs[s.sub] = s;
    for (const s of gaiPillarSubs(dim, country, year)) {
      const ps = prevSubs[s.sub];
      rows.push({ kind: 'leaf', label: s.sub, dim, sub: s.sub, rank: s.rank, score: s.score, prevRank: ps ? ps.rank : null, prevScore: ps ? ps.score : null, hasDef: !!s.def, def: s.def });
    }
  }
  return rows;
}
...
/* R8 Change 3 — rolling 5-year window ending at the latest year with GARI Overall
   data, instead of a hardcoded 2020-2024 range (which silently dropped 2025 once
   it landed in Supabase). Callers can still pass explicit fromY/toY to override. */
function gariOverallSeries(country, fromY, toY) {
  if (toY == null) {
    const yrs = gariYears(); // sorted desc, >=2020 only
    toY = yrs.length ? yrs[0] : 2024;
  }
  if (fromY == null) fromY = toY - 4;
...
const AII_ICONS = {
  'Research & Development': '📊', 'Economy': '💼', 'Education': '🎓',
  'Policy and Governance': '🗳', 'Public Opinion': '💬', 'Responsible AI': '⚖️',
};
/* many raw sub_indicator strings carry long time-series-window / geography
   suffixes (e.g. "AI job postings (% of all job postings) by select
   Geographic areas, 2014-2024") — show only the meaningful first clause. */
function abbreviateSubIndicator(name) {
  return name.replace(/ by .*/i, '').replace(/ \(\d{4}-.*/i, '').replace(/ by country.*/i, '').trim();
}
function aiiYears() {
  const set = new Set();
  for (const r of RECS) if (r.p === AII_PUB) set.add(r.y);
  const arr = [...set].sort((a, b) => b - a);
  return arr.length ? arr : [2025, 2024, 2023];
}
/* Singapore's time-series rows carry a suffixed country string, e.g.
   "Singapore (2024-01)", alongside plain point-in-time "Singapore" rows —
   match both with a prefix check (equivalent to SQL ilike 'Singapore%'). */
/* Some countries appear under multiple spellings across data vintages —
   treat these as the same country everywhere AII rows are matched. */
const AII_COUNTRY_ALIASES = {
...
function aiiCountryAliasGroup(country) {
  for (const canon in AII_COUNTRY_ALIASES) if (AII_COUNTRY_ALIASES[canon].includes(country)) return AII_COUNTRY_ALIASES[canon];
  return [country];
}
function aiiCountryMatches(rowCountry, country) {
  if (!rowCountry) return false;
  return aiiCountryAliasGroup(country).some(v => rowCountry === v || rowCountry.startsWith(v + ' ('));
}
function aiiCountryBase(c) {
  const i = c.indexOf(' (');
  const base = i === -1 ? c : c.slice(0, i);
  for (const canon in AII_COUNTRY_ALIASES) if (AII_COUNTRY_ALIASES[canon].includes(base)) return canon;
  return base;
}
function aiiCountries(year) {
  const set = new Set();
  for (const r of RECS) if (r.p === AII_PUB && r.y === year && r.c) set.add(aiiCountryBase(r.c));
  const arr = [...set].sort((a, b) => a.localeCompare(b));
  return arr.length ? arr : ['Singapore'];
}
/* pillars present for a given year, in canonical order (falls back to the full
   canonical list when no data is loaded yet, so the UI stays usable). */
function aiiPillars(year) {
  const present = new Set();
  for (const r of RECS) if (r.p === AII_PUB && r.y === year) present.add(r.i);
  const ordered = AII_PILLARS.filter(p => present.has(p));
  for (const p of present) if (!ordered.includes(p)) ordered.push(p);
  return ordered.length ? ordered : AII_PILLARS;
}
/* de-duplicated sub-indicator rows for a pillar/country/year — time-series rows
   sharing one sub_indicator name collapse to a single (latest-encountered) row. */
function aiiSubRows(pillar, country, year) {
  const map = new Map();
  for (const r of RECS) if (r.p === AII_PUB && r.i === pillar && r.y === year && r.s && aiiCountryMatches(r.c, country)) map.set(r.s, r);
  return [...map.values()];
}
/* distinct sub-indicator names tracked for a pillar/country in a given year —
   must filter by country so counts match what's actually shown for that country. */
function aiiPillarSubIndicators(pillar, year, country) {
  const set = new Set();
  for (const r of RECS) if (r.p === AII_PUB && r.i === pillar && r.y === year && r.s && (!country || aiiCountryMatches(r.c, country))) set.add(r.s);
  return [...set];
}
/* pillar cards are navigation cards, not metric cards: AII's sub-indicators are
   heterogeneous (different units/scales) so there is no single representative
...
function aiiSubTable(pillar, country, year) {
  const rows = aiiSubRows(pillar, country, year);
  const prevRows = aiiSubRows(pillar, country, year - 1);
  const prevMap = new Map(prevRows.map(r => [r.s, r]));
  return rows.map(r => ({ sub: r.s, score: r.sc, unit: r.u, prevScore: prevMap.has(r.s) ? prevMap.get(r.s).sc : null }))
    .sort((a, b) => a.sub.localeCompare(b.sub));
}
...
Object.assign(window, {
  ...
  attachYoY, attachYoYGariOverall,
  ordinal, formatOrdinal, stripPublishedBy, flagUrl, fmtScore, defOf, overallSeries, yearsFor, countriesFor,

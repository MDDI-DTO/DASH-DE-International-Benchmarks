...
/* ============================================================
   Change 2 (Round 5) — AII static placeholder card
   Hardcoded (no Supabase data yet). Non-interactive: no expand, no
   pillar rows, no hyperlink. Replaced by a standard OvCard once AII
   data lands — no grid change needed at that point.
   ============================================================ */
function OvPlaceholderCard() {
  return (
    <div className="ovx-card ovx-card-placeholder">
      <div className="ovx-accent" style={{ background: AII_META.accentColour, width: '4px' }} />
      <div className="ovx-title">Artificial Intelligence Index (AII)</div>
      <div className="ovx-desc">This publication covers the state and trajectory of artificial intelligence through a cross-sector lens, tracking data on AI research, technical performance, economy, education, policy, and ethics.</div>
      <div className="ovx-src">
        <a className="ovx-src-link" style={{ color: AII_META.accentColour }} href={AII_META.url} target="_blank" rel="noopener noreferrer">{AII_META.publisher}</a>
        <span className="ovx-src-sep">·</span><span>{AII_META.frequency}</span>
      </div>
      <div className="ovx-ph-body">
        <div className="ovx-ph-badge">🔜 Coming soon</div>
        <p className="ovx-ph-text">Coming soon — this index will be incorporated into the dashboard in a future update.</p>
      </div>
    </div>
  );
}
      ...
      {Object.keys(OV_PUBS).map(domain => (
        <section className="ov-domain" key={domain}>
          ...
          <div className="ovx-grid">
            ...
            {domain === 'AI' && <OvPlaceholderCard key="AII-placeholder" />}
          </div>
        </section>
      ))}

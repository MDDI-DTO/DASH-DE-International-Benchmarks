...
const OV_META = {
  ...
  'Global AI Index': { publisher: 'Tortoise Media & The Observer', frequency: 'Annual' },
...
/* Change 1 (Round 7 OV1) — reverted to the three-paragraph intro (Round 4 version).
   Static editorial text (not Supabase data). Domain names are bolded inline via <strong>. */
const OVERVIEW_INTRO_TEXT = [
  <>This dashboard consolidates Singapore’s performance across leading international benchmark publications in the Digital Economy and AI domains, providing a unified view of rankings, trends, and pillar-level detail.</>,
  <>For the <strong>Digital Economy</strong> domain, we draw on the IMD World Competitiveness Yearbook (IMD WCY) and the IMD World Digital Competitiveness Ranking (IMD WDC), both published by the IMD World Competitiveness Center, and the Network Readiness Index (NRI) published by the Portulans Institute.</>,
  <>For the <strong>AI</strong> domain, we draw on the Global AI Index (GAI) by Tortoise Media and The Observer, the Government AI Readiness Index (GARI) by Oxford Insights, and the Artificial Intelligence Index (AII) by Stanford University.</>,
];

/* OV14 — static per-publication descriptions shown below the Overview card title.
   GAI/GARI intentionally omitted (copy not yet provided). */
const CARD_DESCRIPTIONS = {
  'NRI': 'This publication covers how well countries leverage network technologies (ICT) for economic and social development across four pillars (Technology, People, Governance, and Impact).',
  'IMD WCY': 'This publication covers the overall economic competitiveness of nations across four pillars (Economic Performance, Government Efficiency, Business Efficiency, and Technological Infrastructure). We have identified Technological Infrastructure as the pillar of focus in this dashboard due to its relevance to Singapore.',
  'IMD WDC': 'This publication covers the capacity and readiness of economies to adopt and explore digital technologies across three pillars (Knowledge, Technology, and Future Readiness).',
  'Global AI Index': "This publication covers countries' AI investment, innovation, and implementation capacity across three pillars (Implementation, Innovation, Investment). Each pillar is further broken down into sub-pillars, as detailed in the table below.",
  'Government AI Readiness Index': "This publication covers governments' capacity to harness AI for public benefit across six pillars (Policy Capacity, Governance, AI Infrastructure, Public Sector Adoption, Development and Diffusion, and Resilience).",
};

/* R6 Change 4b — static grouping of GAI sub-pillars under their parent pillar, used to
   render pillar header rows in the GAI card's table. Rank/score values stay live from
   Supabase; only this grouping structure is hardcoded. */
const GAI_PILLAR_GROUPING = {
...
/* Pick legible text colour for a label sitting on top of `hex` (G3 inside-bar labels):
   dark ink on light fills, white on dark fills, by perceived luminance. */
function ovTextOn(hex) {
  const n = parseInt(hex.slice(1), 16), r = (n >> 16) & 255, g = (n >> 8) & 255, b = n & 255;
  return (0.299 * r + 0.587 * g + 0.114 * b) / 255 > 0.62 ? '#23202e' : '#ffffff';
}
...
function ovOverall(cfg) {
  ...
  if (o.kind === 'gai') {
    ...
    const prev = (y != null && window.gaiOverallRow) ? window.gaiOverallRow('Singapore', y - 1) : null;
    return { rank: row ? row.r : null, prev: prev ? prev.r : null, year: y };
  }
  if (o.kind === 'gari') {
...
/* ---------- model builders ---------- */
/* GAI Singapore sub-pillar set ("{dimension}::{sub}") for a year — used to confirm the
   prior-year framework matches before showing YoY (mirrors ovPillarsComparable). */
function ovGaiSubSet(year) {
  const set = new Set();
  for (const r of (window.RECS || [])) {
    if (r.p === 'Global AI Index' && r.c === 'Singapore' && r.y === year
...
function ovBuildGAI(cfg) {
  ...
  // prior-year rank lookup keyed by "{dimension}::{sub}"
  const prevRank = {};
  for (const r of (window.RECS || [])) {
    if (r.p === 'Global AI Index' && r.c === 'Singapore' && r.y === prevYear
        && r.i !== 'Overall' && (r.t === '' || r.t == null) && r.s && r.s !== 'Overall' && r.r != null) {
      prevRank[r.i + '::' + r.s] = r.r;
    }
  }
  const rowsRaw = [];
  for (const r of (window.RECS || [])) {
    if (r.p === 'Global AI Index' && r.c === 'Singapore' && r.y === latestYear
      ...
      rowsRaw.push({
        ...
        cur: r.r, prev: (prevRank[key] != null ? prevRank[key] : null), score: r.sc, series: [{ y: r.y, r: r.r }],
      });
    }
  }
  const rows = rowsRaw.sort((a, b) => a.cur - b.cur);
  // OV2 — only show prior-year data once the 2023 sub-pillar set structurally matches 2024's.
  const curSet = ovGaiSubSet(latestYear), prevSet = ovGaiSubSet(prevYear);
  const comparable = curSet.size > 0 && prevSet.size === curSet.size && [...curSet].every(k => prevSet.has(k));
  if (!comparable) for (const r of rows) r.prev = null;
  const ov = ovOverall(cfg);
  const overallComparable = ov.rank != null && ov.prev != null;
  return {
    cfg, latestYear, prevYear, multiYear: comparable, hasPrevYear: comparable, comparable,
    overallComparable, pillarYoyUnavailable: false,
    years: [latestYear], yearSpan: String(latestYear), overall: ov, rows,
    highlights: ovHighlights(cfg, rows, latestYear, comparable ? prevYear : null, comparable),
  };
}
...
function ovBuildPub(cfg) {
  ...
  // OV3 — GARI's Overall (publication-level) rank IS cross-era comparable even when
  // its pillar framework is not; decouple the headline YoY from pillar comparability.
  const ov = ovOverall(cfg);
  const isGari = ((cfg.overall || {}).kind === 'gari');
  const overallComparable = isGari ? (ov.rank != null && ov.prev != null) : comparable;
  const pillarYoyUnavailable = isGari && !comparable;

  return {
    cfg, latestYear, prevYear, multiYear, hasPrevYear, comparable, overallComparable, pillarYoyUnavailable,
    years: trendYears, yearSpan,
    overall: ov, rows: sortedRows,
...
/* Shared rank-label drawer for the Chart.js LINE trend charts (Part A / F / G).
   styleFn(dataset) returns {font, color, offset}. No global padding is reserved (F2):
   an isolated point's label sits above by default, flipped below only if it would
   clip the top edge. A CLUSTER converging at one x has its labels ALTERNATE sides
   (best above, next below, …) so neighbours never stack near an edge (F4/G2); any
   residual same-side pair is stacked outward. Every label gets a white halo so it
   stays legible where two columns' labels pass close (G2). */
function ovDrawRankLabels(chart, styleFn) {
  const { ctx, chartArea } = chart;
  const LBL_H = 12, GAP = 3;
  const items = [];
  chart.data.datasets.forEach((dataset, i) => {
    ...
    meta.data.forEach((el, j) => {
      ...
      items.push({ x: el.x, y: el.y, value, font: st.font, color: st.color, offset: st.offset || 6, placement: 'above', stack: 0 });
    });
  });
  // Group by rounded x (one year column — all line points at a year share an x).
  const groups = new Map();
  for (const it of items) { const k = Math.round(it.x); if (!groups.has(k)) groups.set(k, []); groups.get(k).push(it); }
  for (const arr of groups.values()) {
    arr.sort((a, b) => a.y - b.y); // top-most point first
    let ci = 0;
    while (ci < arr.length) {
      let cj = ci;
      while (cj + 1 < arr.length && (arr[cj + 1].y - arr[cj].y) < (LBL_H + GAP)) cj++;
      const cluster = arr.slice(ci, cj + 1);
      if (cluster.length === 1) {
        const it = cluster[0];
        it.placement = (it.y - it.offset - LBL_H) < chartArea.top ? 'below' : 'above';
      } else {
        // Alternate sides across the converging cluster (F4): best above, next below, …
        cluster.forEach((it, k) => { it.placement = (k % 2 === 0) ? 'above' : 'below'; });
        // Edge-correct: if a chosen side would clip, force the other side.
        cluster.forEach(it => {
          if (it.placement === 'above' && (it.y - it.offset - LBL_H) < chartArea.top) it.placement = 'below';
          else if (it.placement === 'below' && (it.y + it.offset + LBL_H) > chartArea.bottom) it.placement = 'above';
        });
        // Stack same-side pairs outward so they never overprint (mid-chart fallback).
        const above = cluster.filter(it => it.placement === 'above').sort((a, b) => b.y - a.y);
        const below = cluster.filter(it => it.placement === 'below').sort((a, b) => a.y - b.y);
        above.forEach((it, k) => { it.stack = k; });
        below.forEach((it, k) => { it.stack = k; });
      }
      ci = cj + 1;
    }
  }
  for (const it of items) {
    ...
    let y;
    if (it.placement === 'above') { ctx.textBaseline = 'bottom'; y = it.y - it.offset - it.stack * (LBL_H + GAP); }
    else { ctx.textBaseline = 'top'; y = it.y + it.offset + it.stack * (LBL_H + GAP); }
    ctx.lineJoin = 'round'; ctx.lineWidth = 3; ctx.strokeStyle = 'rgba(255,255,255,0.92)';
    ctx.strokeText(ovOrd2(it.value), it.x, y);
    ctx.fillStyle = it.color;
    ctx.fillText(ovOrd2(it.value), it.x, y);
    ctx.restore();
  }
}

/* NRI combined bar+line label drawer (Part G3). Up to four pillar bars + one overall
   line can compete at a single year, so a generic two-series flip isn't enough. Bar
   labels render INSIDE their own bar (contrast-aware ink) when the bar is tall enough
   — so they never collide horizontally with adjacent bars — and just above the bar
   otherwise; the Overall NRI line is the headline series and keeps an above/below
   point label drawn on top of everything, each with a white halo for legibility. */
function ovDrawNRILabels(chart, lineStyle, barStyleFn) {
  const { ctx, chartArea } = chart;
  const LBL_H = 12;
  const getP = (el, keys) => (el.getProps ? el.getProps(keys, true) : el);
  chart.data.datasets.forEach((dataset, di) => {
    if (dataset.type !== 'bar') return;
    const meta = chart.getDatasetMeta(di);
    if (!meta || meta.hidden) return;
    const st = barStyleFn(dataset);
    meta.data.forEach((el, j) => {
      const value = dataset.data[j];
      if (value == null) return;
      const p = getP(el, ['x', 'y', 'base']);
      const barTop = Math.min(p.y, p.base), barBot = Math.max(p.y, p.base);
      const txt = ovOrd2(value);
      ctx.save();
      ctx.font = st.font;
      ctx.textAlign = 'center';
      if (barBot - barTop >= 18) {
        // Tall enough: label sits INSIDE near the bar's top (G3), staggered by the
        // bar's own height so different ranks separate vertically; contrast-aware ink.
        ctx.fillStyle = ovTextOn(st.color);
        ctx.textBaseline = 'top';
        ctx.fillText(txt, p.x, barTop + 3);
      } else {
        const y = Math.max(barTop - 3, chartArea.top + LBL_H);
  ...
  const li = chart.data.datasets.findIndex(d => d.type === 'line');
  if (li >= 0) {
    const meta = chart.getDatasetMeta(li);
    const ds = chart.data.datasets[li];
    meta.data.forEach((el, j) => {
      const value = ds.data[j];
      if (value == null) return;
      const p = getP(el, ['x', 'y']);
      const above = (p.y - lineStyle.offset - LBL_H) >= chartArea.top;
      const y = above ? p.y - lineStyle.offset : p.y + lineStyle.offset;
      ctx.save();
      ctx.font = lineStyle.font;
      ctx.textAlign = 'center';
      ctx.textBaseline = above ? 'bottom' : 'top';
      ctx.lineJoin = 'round'; ctx.lineWidth = 3.5; ctx.strokeStyle = 'rgba(255,255,255,0.96)';
      ctx.strokeText(ovOrd2(value), p.x, y);
      ctx.fillStyle = lineStyle.color;
      ctx.fillText(ovOrd2(value), p.x, y);
      ctx.restore();
    });
  }
}
...
function OvLineChart({ model }) {
  ...
  React.useEffect(() => {
    ...
    // OV1 — axis always starts at 0; increment is data-driven (1 for tight ranges, else 2)
    // and the max is rounded up to a whole number of increments so gridlines land cleanly.
    const _mx = (shownRanks.length ? Math.max(...shownRanks) : 4) + 1;
    const _inc = _mx <= 6 ? 1 : 2;
    const maxRank = Math.ceil(_mx / _inc) * _inc;
    const rankLabels = {
      id: 'rankLabels',
    ...
    chartRef.current = new window.Chart(ref.current.getContext('2d'), {
      ...
      options: {
        ...
        layout: { padding: { top: 24, right: 34, bottom: 18, left: 8 } },
        interaction: { mode: 'nearest', intersect: false },
        plugins: {
          legend: { display: false },
          tooltip: {
            backgroundColor: '#15233a', padding: 9, cornerRadius: 7, titleFont: { size: 13 }, bodyFont: { size: 13 }, displayColors: false,
            callbacks: { title: items => 'Year ' + items[0].label, label: ctx => `${ctx.dataset.label} \u00b7 Rank ${ctx.parsed.y}` },
          },
        },
        scales: {
          y: {
            reverse: true, min: 0, max: maxRank,
            title: { display: true, text: 'Rank', font: { size: 12 }, color: '#94a1b0' },
            ticks: { stepSize: _inc, font: { size: 12 }, color: '#94a1b0', precision: 0,
              callback: v => (v === 0 ? '' : v) },
            grid: { color: 'rgba(16,40,64,0.06)' },
          },
          x: { ticks: { font: { size: 12 }, color: '#94a1b0', maxRotation: 0, autoSkipPadding: 12 }, grid: { display: false } },
        },
      },
    });
...
function OvNRIGroupedChart({ model }) {
  ...
  React.useEffect(() => {
    ...
    // OV1 — 0-origin axis, data-driven increment, max rounded to a whole number of increments.
    const _mx = (allVals.length ? Math.max(...allVals) : 5) + 1;
    const _inc = _mx <= 6 ? 1 : 2;
    const maxRank = Math.ceil(_mx / _inc) * _inc;
    const rankLabels = {
      id: 'nriComboLabels',
      afterDatasetsDraw(chart) {
        ovDrawNRILabels(chart,
          { font: '700 12px ' + getComputedStyle(document.body).fontFamily, color: OV_NRI_OVERALL_COLOR, offset: 9 },
          (dataset) => ({ font: '600 11px ' + getComputedStyle(document.body).fontFamily, color: dataset._labelColor || '#4b5563' }));
      },
    };
    chartRef.current = new window.Chart(ref.current.getContext('2d'), {
      ...
      options: {
        ...
        plugins: {
          legend: { display: false },
          tooltip: {
            backgroundColor: '#15233a', padding: 9, cornerRadius: 7, titleFont: { size: 13 }, bodyFont: { size: 13 },
        ...
        scales: {
          y: {
            reverse: false, min: 0, max: maxRank,
            title: { display: true, text: 'Rank (lower = better)', font: { size: 12 }, color: '#94a1b0' },
            ticks: { stepSize: _inc, font: { size: 12 }, color: '#94a1b0', precision: 0,
              callback: v => (v === 0 ? '' : v) }, grid: { color: 'rgba(16,40,64,0.06)' },
          },
          x: { ticks: { font: { size: 13, weight: '500' }, color: '#4b5563', maxRotation: 0, autoSkip: false }, grid: { display: false } },
        },
      },
    });
    return () => { if (chartRef.current) chartRef.current.destroy(); };
...
/* Shared pillar row — aligned name | rank | YoY columns across all cards.
   (Retained for reference; Overview cards now render OvPillarTable below.) */
...
/* ============================================================
   Fix 5 — Pillar rank-by-year table
   Columns: INDICATOR | RANK {prevYear} | RANK {year}. The current-year value keeps a
   small supplementary direction arrow (per Fix 2 sizing/bolding). When no comparable
   prior year exists (GAI single year, GARI cross-era), the prev column collapses away.
   ============================================================ */
function ovDirArrow(cur, prev) {
  if (cur == null || prev == null) return null;
  if (cur < prev) return <span className="ovx-tarrow up" aria-label="improved">{'\u2191'}</span>;
  if (cur > prev) return <span className="ovx-tarrow down" aria-label="declined">{'\u2193'}</span>;
  return <span className="ovx-tarrow flat" aria-label="unchanged">{'\u2014'}</span>;
}
/* R6 Change 4b — groups GAI's flat sub-pillar rows into pillar header + nested
   sub-pillar rows per GAI_PILLAR_GROUPING. Falls back gracefully if a sub-pillar
   isn't found in the live rows (still renders header, just skips the missing row). */
function ovGaiGroupedRows(rows) {
  const out = [];
  for (const pillar of Object.keys(GAI_PILLAR_GROUPING)) {
    out.push({ type: 'header', label: pillar });
    for (const sub of GAI_PILLAR_GROUPING[pillar]) {
      const r = rows.find(x => x.sub === sub || x.label === sub);
      if (r) out.push({ type: 'row', r, nested: true, pillar });
    }
  }
  return out;
}
function OvPillarTable({ model, onNavigate }) {
  const { cfg } = model;
  const showPrev = model.comparable && model.prevYear != null;
  return (
    <table className="ovx-ptable">
      <thead>
        <tr>
          <th className="ovx-pt-ind">Pillar</th>
          <th className="ovx-pt-rank">Rank {model.latestYear}</th>
          {showPrev && <th className="ovx-pt-rank">Rank {model.prevYear}</th>}
        </tr>
      </thead>
      <tbody>
        {(cfg.pub === 'Global AI Index' ? ovGaiGroupedRows(model.rows) : model.rows.map(r => ({ type: 'row', r }))).map((entry, idx) => {
          if (entry.type === 'header') {
            return (
              <tr key={'hdr-' + entry.label} className="ovx-pt-headerrow">
                <td className="ovx-pt-ind" colSpan={1}>{entry.label}</td>
                <td className="ovx-pt-rank"></td>
                {showPrev && <td className="ovx-pt-rank"></td>}
              </tr>
            );
          }
          const r = entry.r;
          const icon = ovIcon(cfg.pub, r.label);
          return (
            <tr key={r.indicator} className={entry.nested ? 'ovx-pt-nested' : ''}>
              <td className="ovx-pt-ind">
                {/* Fix 7/9/11 — icon + name + ⓘ live in one flex wrapper INSIDE the
                   cell (not on the <td> itself, which broke the row divider per Fix 10).
                   The ⓘ flows inline right after the name text, so it sits beside the
                   last word even when the name wraps to two lines. */}
                <div className="ovx-ind-cell">
                  {icon && <span className="ovx-pr-icon" aria-hidden="true">{icon}</span>}
                  <span className="ovx-ind-text">
                    {/* R7 OV2 — pillar name links to its deep-dive tab (falls back to
                       the publication's Overall tab when no deep-dive exists). */}
                    <a className="ovx-pr-label ovx-pr-navlink" href="#" onClick={e => { e.preventDefault(); onNavigate && onNavigate(cfg.pub, entry.pillar || r.label, entry.pillar ? r.label : undefined); }}>{r.label}</a>
                    {r.definition ? <>{'\u00A0'}<OvInfoIcon definition={r.definition} /></> : null}
                  </span>
                </div>
              </td>
              <td className="ovx-pt-rank">
                <span className="ovx-pt-cur">{r.cur}{ovOrd(r.cur)}{showPrev && ovDirArrow(r.cur, r.prev)}</span>
              </td>
              {showPrev && (
                <td className="ovx-pt-rank">
                  <span className="ovx-pt-val">{r.prev != null ? <>{r.prev}{ovOrd(r.prev)}</> : '\u2013'}</span>
                </td>
              )}
            </tr>
          );
...
/* Fix 4 — headline rank's prior-year line: state the actual prior-year rank (with a
   direction arrow) rather than a bare delta. When there's no comparable prior year,
   show explanatory text and no arrow. */
function ovOverallPrior(ov, comparable) {
  if (ov.rank == null || ov.prev == null || !comparable) {
    return <span className="ovx-prior-none">No prior year rank available</span>;
  }
  const prevYear = ov.year != null ? ov.year - 1 : null;
  const dir = ov.rank < ov.prev ? 'up' : ov.rank > ov.prev ? 'down' : 'flat';
  const arrow = dir === 'up' ? '\u2191' : dir === 'down' ? '\u2193' : '\u2014';
  return (
    <span className={'ovx-prior ' + dir}>
      <span className="ovx-prior-arrow">{arrow}</span>
      <span className="ovx-prior-txt">{ov.prev}{ovOrd(ov.prev)} in {prevYear}</span>
    </span>
  );
}
...
/* ============================================================
   Publication card
   ============================================================ */
function OvCard({ model, open, onToggle, onNavigate }) {
  const { cfg } = model;
  ...
  return (
    <div className={'ovx-card' + (open ? ' open' : '')} ref={cardRef}>
      ...
      <div className="ovx-title">
        <a className="ovx-title-link" href="#" style={{ '--hc': cfg.accent }}
          onClick={e => { e.preventDefault(); onNavigate && onNavigate(cfg.pub); }}>{cfg.full} ({cfg.short})</a>
      </div>

      {CARD_DESCRIPTIONS[cfg.pub] && <div className="ovx-desc">{CARD_DESCRIPTIONS[cfg.pub]}</div>}

      <div className="ovx-src">
        {cfg.pub === 'Global AI Index' ? (
          <>
            <a className="ovx-src-link" style={{ color: cfg.accent }} href="https://www.tortoisemedia.com/data/global-ai" target="_blank" rel="noopener noreferrer">Tortoise Media</a>
            <span>{' & '}</span>
            <a className="ovx-src-link" style={{ color: cfg.accent }} href="https://observer.co.uk/data/global-ai" target="_blank" rel="noopener noreferrer">The Observer</a>
          </>
        ) : (
          meta.publisher && <a className="ovx-src-link" style={{ color: cfg.accent }} href={url} target="_blank" rel="noopener noreferrer">{meta.publisher}</a>
        )}
      ...
      <div className="ovx-overall">
        ...
        <div className="ovx-overall-meta">
          <div className="ovx-overall-label">Overall{ov.year ? ' \u00b7 ' + ov.year : ''}</div>
          <div className="ovx-overall-arrow">{ovOverallPrior(ov, model.overallComparable != null ? model.overallComparable : model.comparable)}</div>
        </div>
      </div>

      <div className="ovx-pillars">
        <OvPillarTable model={model} onNavigate={onNavigate} />
        {model.pillarYoyUnavailable && (
          <p className="ovx-pillar-note">* Pillar-level YoY comparison unavailable — 2025 introduced a restructured framework</p>
        )}
      </div>
...
/* Metadata for the AII placeholder card — mirrors the OV_META / OV_PUBS accent
   pattern used for live publications. Swap `active` to true and fold this into
   OV_PUBS/OV_META once real AII data lands. */
const AII_META = {
  shortName:    'AII',
  tabLabel:     'AI Index',
  accentColour: '#0F766E',
  accentDark:   '#0D5E57',   // darker shade for text on light backgrounds
  accentLight:  '#F0FDFA',   // pale teal for badge backgrounds when card goes live
  publisher:    'Stanford HAI',
  frequency:    'Annual',
  url:          'https://hai.stanford.edu/ai-index',
  domain:       'AI',
  active:       false,       // placeholder state — set to true when data is ready
};

/* ============================================================
   Change (AII Overview Option A) — informational card, not rank-led.
   The Global AI Vibrancy Tool ranking data has not yet been ingested,
   so this card shows what AII covers + per-pillar indicator counts
   instead of the rank-led format used by the other 5 cards.
   TODO: once ranking data lands (publication = 'Stanford AI Index',
   indicator = 'Overall', rank IS NOT NULL), replace this with a
   standard OvCard-style rank-led card: (1) rank number block,
   (2) pillar rank rows + YoY arrows, (3) trend chart in the expanded
   panel — mirroring the other publications.
   ============================================================ */
function OvAIICard({ onNavigate }) {
  const [open, setOpen] = React.useState(false);
  const year = React.useMemo(() => { const ys = window.aiiYears ? window.aiiYears() : []; return ys.length ? ys[0] : null; }, []);
  const pillarRows = React.useMemo(() => {
    if (year == null || !window.aiiSubRows) return [];
    return (window.AII_PILLARS || []).map(p => ({ pillar: p, count: window.aiiSubRows(p, 'Singapore', year).length })).filter(r => r.count > 0);
  }, [year]);
  const accent = window.AII_ACCENT || AII_META.accentColour;
  const dark = window.AII_DARK || AII_META.accentDark;
  const icons = window.AII_ICONS || {};

  return (
    <div className={'ovx-card' + (open ? ' open' : '')}>
      <div className="ovx-accent" style={{ background: accent }} />
      <div className="ovx-title">
        <a className="ovx-title-link" href="#" style={{ '--hc': dark }}
          onClick={e => { e.preventDefault(); onNavigate && onNavigate('Stanford AI Index'); }}>Artificial Intelligence Index (AII)</a>
      </div>
      <div className="ovx-desc">This publication covers the state and trajectory of artificial intelligence through a cross-sector lens, tracking data on AI research, technical performance, economy, education, policy, and ethics. {window.ExtLinkIcon && <window.ExtLinkIcon url={AII_META.url} color={dark} />}</div>
      <div className="ovx-src">
        <a className="ovx-src-link" style={{ color: dark }} href={AII_META.url} target="_blank" rel="noopener noreferrer">{AII_META.publisher}</a>
        <span className="ovx-src-sep">·</span><span>{AII_META.frequency}</span>
      </div>

      <div className="ovx-pillars">
        <table className="ovx-ptable">
          <thead><tr><th className="ovx-pt-ind">Pillar</th><th className="ovx-pt-rank">Indicators Tracked</th></tr></thead>
          <tbody>
            {pillarRows.length ? pillarRows.map(r => (
              <tr key={r.pillar}>
                <td className="ovx-pt-ind">
                  <div className="ovx-ind-cell">
                    {icons[r.pillar] && <span className="ovx-pr-icon" aria-hidden="true">{icons[r.pillar]}</span>}
                    <span className="ovx-ind-text"><a className="ovx-pr-label ovx-pr-navlink" href="#" onClick={e => { e.preventDefault(); onNavigate && onNavigate('Stanford AI Index', r.pillar); }}>{r.pillar}</a></span>
                  </div>
                </td>
                <td className="ovx-pt-rank"><span className="ovx-pt-cur">{r.count}</span></td>
              </tr>
            )) : (
              <tr><td className="ovx-pt-ind" colSpan={2}>Pending data ingestion</td></tr>
            )}
          </tbody>
        </table>
      </div>
      <div className="card-footnote">* Only indicators that reference Singapore are tracked in this dashboard. No overall pillar ranking is available — the table above shows the number of indicators tracked per pillar instead.</div>

      <button className={'ovx-expand' + (open ? ' open' : '')} onClick={() => setOpen(o => !o)} aria-expanded={open}>
        <span>{open ? 'Hide details' : 'Expand to see more details'}</span>
        <svg className="ovx-chev" viewBox="0 0 24 24" fill="none" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
          <polyline points="6 9 12 15 18 9" />
        </svg>
      ...
      {open && (
        <div className="ovx-panel">
          <div className="aii-ov-note">
            <span className="aii-ov-note-icon" aria-hidden="true">ℹ</span>
            <p>Unlike other publications in this dashboard, the AI Index tracks indicator values rather than consolidated pillar rankings. Explore the Artificial Intelligence Index tab for full indicator detail and country comparisons.</p>
          </div>
          <a className="aii-ov-cta" href="#" style={{ color: dark }} onClick={e => { e.preventDefault(); onNavigate && onNavigate('Stanford AI Index'); }}>{'\u2192'} Go to AI Index tab</a>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="site ov-fluid">
      <div className="ov-intro">
        {OVERVIEW_INTRO_TEXT.map((para, i) => <p key={i}>{para}</p>)}
      </div>
      {Object.keys(OV_PUBS).map(domain => (
        <section className="ov-domain" key={domain}>
          ...
          <div className="ovx-grid">
            ...
            {domain === 'AI' && <OvAIICard key="AII-overview-card" onNavigate={onNavigate} />}
          </div>
        </section>
      ))}

...
/* ---------- G5 — external-link glyph shown after an index/pillar title, opening
   the publication's source URL in a new tab (in addition to the source-line link). */
function ExtLinkIcon({ url, color, title }) {
  if (!url) return null;
  return (
    <a className="hc-extlink" href={url} target="_blank" rel="noopener noreferrer"
      title={title || 'Open source publication (new tab)'} aria-label={title || 'Open source publication in new tab'}
      style={{ color: color || 'currentColor' }} onClick={e => e.stopPropagation()}>
...
/* ---------- classification footer (every panel) ----------
   Source attribution text is derived per-publication (not from PUB_SHORT) and the
   year is always the currently selected filter year — never hardcoded. GAI's
   attributed source itself is year-dependent (title/ownership changed after 2024). */
function getGAISource(year) { return year <= 2024 ? 'Tortoise Media' : 'The Observer'; }
function classFootSource(pub, year) {
  const y = year != null ? ` (${year})` : '';
  if (pub === 'IMD WCY' || pub === 'IMD WDC') return `IMD${y}`;
  if (pub === 'NRI') return `Portulans Institute${y}`;
  if (pub === 'Global AI Index') return `${getGAISource(year)}${y}`;
  if (pub === 'Government AI Readiness Index') return `Oxford Insights${y}`;
  if (pub === 'Stanford AI Index') return `Stanford HAI${y}`;
  return `${(window.PUB_SHORT && window.PUB_SHORT[pub]) || pub}${y}`;
}
function ClassFoot({ pub, year }) {
  return (
    <div className="class-foot">
      <span>Classification: Official (Open)</span>
      <span className="cf-src">Source: {classFootSource(pub, year)}</span>
    </div>
  );
}
  ...
  return (
    <div className="panel header-card imd-header">
      ...
      <div className="hc-left">
        ...
        {def && <div className="hc-def">{def} <ExtLinkIcon url={(window.PUB_URL && window.PUB_URL[pillar.pub])} color={PUB_COLOR[pillar.pub]} /></div>}
        <div className="hc-source"><span className="src-link" style={{ color: PUB_COLOR[pillar.pub] }}>IMD World Competitiveness Center</span> · Annual</div>
      </div>
...
function FilterBar({ years, countries, year, country, onYear, onCountry, yearLocked, note, hint,
                     ...
                     subPillarOptions, subPillarValue, onSubPillar, subPillarAllOption }) {
  return (
        ...
        <div className="flt">
          ...
          <div className="flt-select-wrap">
            <select className="flt-select" value={subPillarValue} onChange={e => onSubPillar(e.target.value)}>
              {subPillarAllOption !== false && <option value="all">All Sub-Pillars</option>}
...
function ChartPanel({ pillar, country, year, data, onPickCountry, selectedSub, onReset }) {
  ...
  const rows = attachYoY(compareTop(pillar.pub, pillar.indicator, year, sub, third, null), pillar.pub, pillar.indicator, year, sub, third);
  const title = selectedSub
    ? <><span className="ind-chip">{selectedSub.label}</span> · Comparison across countries · <span style={{ color: PUB_COLOR[pillar.pub], fontWeight: 600 }}>{country}</span></>
    : <>Comparison across countries · <span style={{ color: PUB_COLOR[pillar.pub], fontWeight: 600 }}>{country}</span></>;
  return (
    <div className="panel">
      <div className="panel-head panel-head-row">
        <div className="ph-titles">
          {selectedSub && (
            <button type="button" className="indicator-back-btn" style={{ color: PUB_COLOR[pillar.pub] }} onClick={onReset}>← Back to sub-pillar overview</button>
          )}
          <div className="panel-title">{title}</div>
          <div className="panel-sub">Click on a country to explore its indicators · Showing {rows.length} countries</div>
        </div>
      </div>
      <div className="panel-body body-scroll">
        <CountryBarChart rows={rows} selectedCountry={country} onPick={onPickCountry} fixedRows />
      </div>
...
Object.assign(window, { ClassFoot, PillarHeaderCard, FilterBar, ChartPanel, ChartViewControl, ChartResetButton, SourceLink, ExtLinkIcon, rankYoYText, PILLAR_CONTEXT, getGAISource, classFootSource });

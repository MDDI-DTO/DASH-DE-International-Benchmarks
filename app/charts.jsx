...
function CountryBarChart({ rows, selectedCountry, onPick, interactive = true, fixedRows = false, highlightColor, widthByScore = false }) {
  ...
  const hasYoY = rows.some(r => r.prevRank != null);
  return (
    <div className={'barchart' + (fixedRows ? ' barchart-scroll' : '')}>
      {hasYoY && <div className="bar-chart-yoy-header">YoY (Rank)</div>}
      {rows.map((r, i) => {
        ...
        return (
          <div key={r.country + i}
            ...
            {r.prevRank != null && (() => {
              const d = r.prevRank - r.rank; // positive = improved (rank fell)
              const cls = d > 0 ? 'up' : d < 0 ? 'down' : 'flat';
              const txt = d > 0 ? `\u2191 ${d}` : d < 0 ? `\u2193 ${Math.abs(d)}` : '\u2014';
              return <span className={'bar-yoy-indicator ' + cls}>{txt}</span>;
            })()}
          </div>
        );
      })}
...
/* Part A1 — keep a centre-anchored SVG label fully inside the [0,W] drawing box.
   The outer <svg> clips overflow (UA default), so a label whose half-width spills
   past the right (or left) edge renders truncated ("2026"->"202", "2nd"->"2n").
   We estimate the text's half-width from its character count at the given font
   size and clamp the centre x so the whole glyph run stays visible. */
function clampLabelX(cx, text, fontPx, W, pad = 2) {
  const half = (String(text).length * fontPx * 0.62) / 2;
  return Math.max(half + pad, Math.min(W - half - pad, cx));
}
...
function RankTrendChart({ series, country, color = '#0A6EA8' }) {
  ...
  // A1: a touch more right padding so the rightmost flag + label sit clear of the edge.
  const W = Math.max(220, size.w), H = Math.max(110, size.h), pl = 24, pr = 28, ptop = 22, pbot = 38;
      ...
      {pts.map((p, i) => {
        ...
        return (
          <g key={i}>
            ...
            <text className="trend-rank-label" x={clampLabelX(x(i), `${p.r}${ordinal(p.r)}`, 9, W)} y={y(p.r) + RAD + 13}>{p.r}{ordinal(p.r)}</text>
            <text className="trend-year-label" x={clampLabelX(x(i), p.y, 8, W)} y={H - 4}>{p.y}</text>
          </g>
        );
      })}
    </svg>
...
function NRIProfileChart({ mode, profile, compare }) {
      ...
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 }}>
        <div style={{ flex: 1, minHeight: 0 }}>
          <svg width="100%" height="100%" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="xMidYMid meet" style={{ display: 'block' }}>
            <text x="6" y="16" className="trend-year-label" style={{ fontSize: 13 }}>Rank (lower = better)</text>
            <line x1={pl} y1={ptop} x2={pl} y2={H - pbot} stroke="rgba(16,40,64,0.12)" strokeWidth="1" />
            <text x={pl - 9} y={y(rMin) + 4} textAnchor="end" className="trend-year-label" style={{ fontSize: 13 }}>{rMin}</text>
            <text x={pl - 9} y={y(rMax) + 4} textAnchor="end" className="trend-year-label" style={{ fontSize: 13 }}>{rMax}</text>
            {rows.map((r, i) => {
              ...
              return (
                <g key={r.country} onMouseMove={e => setTip({ x: e.clientX, y: e.clientY, t: `${r.country} · Rank ${r.rank}${ordinal(r.rank)} · Score: ${fmtScore(r.score)}` })} onMouseLeave={() => setTip(null)} style={{ cursor: 'default' }}>
                  ...
                  <text x={cx} y={cy + rad + 15} className="trend-rank-label" style={{ fontSize: 12 }}>{r.rank}{ordinal(r.rank)}</text>
                  <text x={cx} y={H - pbot + 32} className="trend-year-label" style={{ fontSize: 13, fontWeight: me ? 700 : 400 }}>{iso || r.country.slice(0, 3).toUpperCase()}</text>
                </g>
              );
            })}
          </svg>
  ...
  // A2: within each sub-pillar column, stack the per-country rank labels so two
  // countries sharing the same (or a near-equal) rank don't print on top of each
  // other. Labels are placed best-rank first and pushed downward to keep a min gap.
  const colLabelY = subs.map((_, i) => {
    const entries = series
      .map((ser, si) => ({ si, rank: ser.pts[i] && ser.pts[i].rank, rad: ser.isMe ? 12 : 10 }))
      .filter(e => e.rank != null)
      .sort((a, b) => a.rank - b.rank);
    const placed = {}; let lastY = -Infinity;
    for (const e of entries) {
      let ly = y(e.rank) + e.rad + 14;
      if (ly - lastY < 12) ly = lastY + 12;
      lastY = ly; placed[e.si] = ly;
    }
    return placed;
  });
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 }}>
      <div style={{ flex: 1, minHeight: 0 }}>
        <svg width="100%" height="100%" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="xMidYMid meet" style={{ display: 'block' }}>
          ...
          <text x={pl - 9} y={y(rMax) + 4} textAnchor="end" className="trend-year-label" style={{ fontSize: 13 }}>{rMax}</text>
          {subs.map((s, i) => (
            <text key={s} x={x(i)} y={H - pbot + 30} className="trend-year-label" style={{ fontSize: 13 }}>{label(s)}</text>
          ))}
          {series.map((ser, si) => {
          ...
          {series.map((ser, si) => ser.pts.map((p, i) => {
            ...
            return (
              <g key={id} onMouseMove={e => setTip({ x: e.clientX, y: e.clientY, t: `${ser.country} · ${label(p.sub)} · Rank ${p.rank}${ordinal(p.rank)} · Score: ${fmtScore(p.score)}` })} onMouseLeave={() => setTip(null)} style={{ cursor: 'default' }}>
                ...
                <text x={cx} y={colLabelY[i] && colLabelY[i][si] != null ? colLabelY[i][si] : cy + rad + 14} className="trend-rank-label" style={{ fontSize: 12 }}>{p.rank}{ordinal(p.rank)}</text>
              </g>
            );
          }))}
        </svg>
...
Object.assign(window, { CountryBarChart, RankTrendChart, NRIProfileChart, clampLabelX });

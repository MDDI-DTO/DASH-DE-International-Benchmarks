      ...
      {pts.map((p, i) => {
        ...
        return (
          <g key={i}>
            ...
            <text x={x(i)} y={y(p.r) - RAD - 4} textAnchor="middle" style={{ fontSize: 9, fontWeight: 600, fill: 'var(--color-text-secondary)' }}>{p.r}{ordinal(p.r)}</text>
            <text x={x(i)} y={H - 5} textAnchor="middle" style={{ fontSize: 8, fill: 'var(--color-text-tertiary)' }}>{p.y}</text>
          </g>
        );
      })}
    </svg>

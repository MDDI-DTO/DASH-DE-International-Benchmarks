    ...
    <>
      ...
      {banner && (
        <div className="gari-fw-banner">
          ...
          <span className="gfb-text">The 2025 framework was significantly restructured. Sub-pillar ranks and scores are not directly comparable to 2020–2024 values. Overall rank remains a valid yearly trend for comparison.</span>
  ...
  return (
    <>
      ...
      <div className="panel ov-imd-chart gari-chart">
        <div className="panel-head panel-head-row">
          <div className="ph-titles">
            <div className="panel-title">{P} – Country Rank Comparison ({year})</div>
            <div className="panel-sub">Click on a country to explore its ranking trend · Showing {cmpRows.length} countries</div>
          </div>
        </div>
        <div className="panel-body">
        ...
        <div className="panel">
          <div className="panel-head panel-head-row">
            <div className="ph-titles">
              ...
              <div className="panel-sub">Click on a country to explore its sub-pillars · Showing {matrix.rows.length} countries</div>
            </div>
          </div>
          <div className="nri-matrix-wrap scroll">
            <GARIMatrixTable matrix={matrix} selectedCountry={country} onPickCountry={setCountry} />
          </div>
          <div className="gari-matrix-foot"><span>{matrix.rows.length} countries shown</span><span>Source: Oxford Insights ({year})</span></div>
        </div>

        {/* sub-indicators table (2-level) */}
        <div className="panel">
          <div className="panel-head">
            <div className="panel-title">Comparison of sub-pillars · <span style={{ color: GARI_DARK, fontWeight: 600 }}>{country}</span></div>
            <div className="panel-sub">Click on a sub-pillar to explore its country comparison</div>
          </div>
          <div className="subtable-wrap">
            <table className="subtable gari-pillar-subtable">
              <thead>
                <tr>
              ...
              <tbody>
                {tableRows.filter(r => r.kind !== 'header').map((r, i) => {
                  if (r.kind === 'overall') {
          ...
          <div className="gari-matrix-foot"><span>Classification: Official (Open)</span><span>Source: Oxford Insights ({year})</span></div>
        </div>
      </div>
...
/* ============================================================
   ...
   ============================================================ */
function GARI2025FrameworkView({ domain, initialPillar }) {
  const YEAR = 2025;
  const [country, setCountry] = React.useState('Singapore');
  ...
  // R14 — one pillar is always active (no "All" aggregate state). Defaults to the
  // pillar requested via Overview-card navigation (initialPillar), falling back to
  // the first pillar when navigated to directly (e.g. from the nav bar).
  const [selPillar, setSelPillar] = React.useState(() => (
    (initialPillar && cards.some(c => c.pillar === initialPillar)) ? initialPillar : (cards[0] ? cards[0].pillar : null)
  ));
  const [selSub, setSelSub] = React.useState('Overall');
  // "View All" — independent toggle scoped to the right-hand table only; does not
  // touch the Pillars filter/selection or which pillar card is highlighted.
  const [viewAll, setViewAll] = React.useState(false);
  React.useEffect(() => {
    if (cards.length && !cards.some(c => c.pillar === selPillar)) {
      setSelPillar(cards[0].pillar); setSelSub('Overall');
    }
  }, [cards.length]); // eslint-disable-line

  // sub-indicators table model — selected pillar's sub-pillars by default, or every
  // pillar stacked with header rows when "View All" is on.
  const tableModel = React.useMemo(() => {
    const rows = [];
    const list = viewAll ? cards : cards.filter(c => c.pillar === selPillar);
    for (const c of list) {
      if (viewAll) rows.push({ kind: 'header', pillar: c.pillar, label: c.pillar });
    ...
    return rows;
  }, [viewAll, selPillar, cards, country]);

  const cmpRows = attachYoY(gariCompare(selPillar, selSub === 'Overall' ? 'Overall' : selSub, YEAR, 'all', country || 'Singapore'), GARI_PUB, selPillar, YEAR, selSub === 'Overall' ? 'Overall' : selSub, '');
      ...
      <div className="nri2-grid">
          ...
          <div className="panel-head panel-head-row">
            <div className="ph-titles">
              <div className="panel-title">
                Comparison of <span style={{ color: 'var(--color-text-primary)' }}>{selPillar}</span>
                {selSub !== 'Overall' && <> · <span style={{ color: GARI_ACCENT, fontWeight: 600 }}>{selSub}</span></>}
                {' '}across countries · <span style={{ color: GARI_DARK, fontWeight: 600 }}>{country}</span>
              </div>
        ...
        <div className="panel">
          <div className="panel-head" style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
            <div>
              <div className="panel-title">Comparison of {viewAll ? 'all sub-pillars' : selPillar + ' sub-pillars'} · <span style={{ color: GARI_DARK, fontWeight: 600 }}>{country}</span></div>
              <div className="panel-sub">Click on a sub-pillar to explore its country comparison</div>
            </div>
            <button type="button" className="gari-viewall-toggle" onClick={() => setViewAll(v => !v)}>

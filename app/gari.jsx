...
function GARI2025FrameworkView({ domain }) {
  ...
  // R13 Change 3 — one pillar is always active (no "All" aggregate state). Defaults
  // to the first pillar on every entry to this tab (component remounts on nav away/
  // back, so a fresh useState default is all that's needed — no session memory).
  const [selPillar, setSelPillar] = React.useState(() => (cards[0] ? cards[0].pillar : null));
